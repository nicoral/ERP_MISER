...
import type { TableColumn, TableAction, TablePaginationProps, TableProps } from '../../types/table';
    ...
    <div className="p-4">
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead>
          <tr>
            {columns.map((col, idx) => (
              <th key={idx} className={col.className || 'px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider'}>{col.header}</th>
            ))}
            {actions && actions.length > 0 && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Acciones</th>}
          </tr>
        </thead>
        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
          {loading ? (
            <TableSkeleton rows={pageSize} columns={columns.length + (actions && actions.length > 0 ? 1 : 0)} />
          ) : (
            data.map((row, rowIdx) => (
              <tr key={String(row[keyField]) || rowIdx}>
                {columns.map((col, colIdx) => (
                  <td key={colIdx} className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                    {col.render ? col.render(row) : (col.accessor ? (row[col.accessor] as React.ReactNode) : null)}
                  </td>
                ))}
            ...
            <div>
              <nav className="relative z-0 inline-flex rounded-md shadow-sm" aria-label="Pagination">
                ...
                {getPaginationItems(pagination.page, pagination.totalPages).map((item, idx) =>
                  item === '...'
                    ? (
                      <span key={`ellipsis-${idx}`} className="mx-1 px-3 py-2 text-gray-400 dark:text-gray-500 select-none">...</span>
                    ) : (
                      <button
                        key={item}
