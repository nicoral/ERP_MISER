...
export const Administration = () => {
  const [selected, setSelected] = useState<"general" | "users" | "audit">("general");

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">{ADMINISTRATION_TEXTS.title}</h1>
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <h2 className="text-lg font-medium text-blue-900 dark:text-blue-100">{ADMINISTRATION_TEXTS.panels.general.title}</h2>
            <p className="mt-2 text-blue-700 dark:text-blue-300">{ADMINISTRATION_TEXTS.panels.general.description}</p>
          </button>
          <button
            type="button"
          ...
          >
            <h2 className="text-lg font-medium text-green-900 dark:text-green-100">{ADMINISTRATION_TEXTS.panels.users.title}</h2>
            <p className="mt-2 text-green-700 dark:text-green-300">{ADMINISTRATION_TEXTS.panels.users.description}</p>
          </button>
          <button
            type="button"
          ...
          >
            <h2 className="text-lg font-medium text-purple-900 dark:text-purple-100">{ADMINISTRATION_TEXTS.panels.audit.title}</h2>
            <p className="mt-2 text-purple-700 dark:text-purple-300">{ADMINISTRATION_TEXTS.panels.audit.description}</p>
          </button>
        </div>
        {/* Formulario debajo */}
