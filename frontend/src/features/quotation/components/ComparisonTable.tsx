...
export const ComparisonTable: React.FC<ComparisonTableProps> = ({
  ...
  const getSupplierServiceItem = (
    ...
    return qs?.supplierQuotation?.supplierQuotationServiceItems?.find(
      (item: SupplierQuotationServiceItem) =>
        item.requirementService.service.id === requirementServiceId
    );
  };
  ...
  const getSelectedServices = () => {
    ...
    return (
      quotation.requirement.requirementServices
        ?.filter(reqService =>
          selectedServiceIds.includes(reqService.service.id)
        )
        .map(reqService => ({
          id: reqService.id,
  ...
  const getSupplierTotal = (supplierId: number): number => {
    ...
    if (qs.supplierQuotation.supplierQuotationServiceItems) {
      const servicesTotal = qs.supplierQuotation.supplierQuotationServiceItems
        .filter(
          (item: SupplierQuotationServiceItem) =>
            selectedServices.includes(item.requirementService.service.id) &&
            item.status === QuotationItemStatus.QUOTED
        )
        .reduce(
  ...
  return (
    <>
      {!selectedArticles.length && !selectedServices.length ? (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 text-center">
          <p className="text-gray-500 dark:text-gray-400">
            No hay artículos o servicios seleccionados para mostrar en la
            comparación.
          </p>
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-2">
            Asegúrate de que el proveedor seleccionado tenga artículos o
            servicios asignados en la selección final.
          </p>
        </div>
      ) : (
