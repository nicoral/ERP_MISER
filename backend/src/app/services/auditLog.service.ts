...
export class AuditLogService {
  ...
  async findAll(
    ...
    if (date) {
      query.where('auditLog.timestamp >= :dateIni AND auditLog.timestamp <= :dateFin', { dateIni: new Date(date).setHours(0, 0, 0, 0), dateFin: new Date(date).setHours(23, 59, 59, 999) });
    }
